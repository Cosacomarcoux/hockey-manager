...
def planificador_datos(partido_id):
    ...
    # (esto considera K, formación, anclas y la prioridad de cada jugadora)
    ...
    if config:
        try:
            ...
            # Si falla el cálculo, dejamos todos en 0 — la UI lo maneja
            minutos_teoricos_por_id = {}
    ...
    for j in jugadoras:
        ...
        # Turnos teóricos = redondeo de minutos_teóricos / duracion_turno
    ...
    for n in range(1, plan.cantidad_turnos + 1):
        asignaciones = AsignacionTurno.query.filter_by(plan_id=plan.id, turno_numero=n).all()
        turnos_data[n] = [
...
def planificador_swap(partido_id):
    ...
    if asign_j1 and asign_j2:
        slot_temp = asign_j1.slot_indice
        asign_j1.slot_indice = asign_j2.slot_indice
        asign_j2.slot_indice = slot_temp
        db.session.commit()
        return jsonify({'ok': True, 'mensaje': f'{j1.nombre} ↔ {j2.nombre} (cambio de posición en cancha)'})

    # Caso 2: una está en el turno, la otra no → swap cancha-banco
    if asign_j1 and not asign_j2:
        # j1 sale, j2 entra en el slot de j1
        ...
        db.session.commit()
        return jsonify({'ok': True, 'mensaje': f'{j1.nombre} ↔ {j2.nombre}'})

    if asign_j2 and not asign_j1:
        # j2 sale, j1 entra en el slot de j2
        ...
        db.session.commit()
        return jsonify({'ok': True, 'mensaje': f'{j2.nombre} ↔ {j1.nombre}'})
