...
def planificador_ver(partido_id):
    """
    Pantalla principal del planificador (Etapa A2 — vacía por ahora).
    Por ahora muestra un resumen del plan generado.
    """
    entrenador = entrenador_actual()
    partido = Partido.query.filter_by(id=partido_id, entrenador_id=entrenador.id).first_or_404()
    plan = PlanificacionPartido.query.filter_by(partido_id=partido.id).first()
    ...
    # Stats de cuántos turnos tiene cada jugadora asignados
    convocadas_ids = {c.jugadora_id for c in partido.convocadas}
    jugadoras = Jugadora.query.filter(
        Jugadora.entrenador_id == entrenador.id,
    ...
    stats_por_jugadora = []
    for j in jugadoras:
        turnos = plan.turnos_de_jugadora(j.id)
        minutos_planeados = len(turnos) * plan.duracion_turno_min
        es_ancla = config and j.id in config.anclas_set
        stats_por_jugadora.append({
            'jugadora': j,
            'cantidad_turnos': len(turnos),
            'turnos': turnos,
            'minutos_planeados': minutos_planeados,
            'es_ancla': es_ancla,
        })

    # Ordenar por minutos planeados desc
    stats_por_jugadora.sort(key=lambda x: x['minutos_planeados'], reverse=True)
