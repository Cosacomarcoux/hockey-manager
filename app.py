...
class AsignacionTurno(db.Model):
    """
    Asignación: jugadora X juega el turno N del plan.
    Una jugadora puede aparecer en varios turnos (multiple rows).
    """
    id = db.Column(db.Integer, primary_key=True)
...
def planificador_configurar(partido_id):
    ...
    if request.method == 'POST':
        try:
            ...
            generar_auto = request.form.get('generar_auto') == '1'
            ...
            if plan is None:
                ...
                if plan.duracion_turno_min != duracion or generar_auto:
                    AsignacionTurno.query.filter_by(plan_id=plan.id).delete()
                    plan.duracion_turno_min = duracion

            db.session.flush()

            # Generar plan automático si se pidió
            if generar_auto:
                jugadoras = Jugadora.query.filter_by(entrenador_id=entrenador.id).all()
                mc = rotacion_adapter.crear_match_controller(partido, jugadoras, config)
                ...
                # Persistir las asignaciones
                for turno_n, ids_jugadoras in plan_dict.items():
                    for jid in ids_jugadoras:
                        db.session.add(AsignacionTurno(
                            plan_id=plan.id, jugadora_id=jid, turno_numero=turno_n
                        ))
...
def planificador_datos(partido_id):
    ...
    - Datos del plan
    - Para cada turno, qué jugadoras lo juegan
    - Datos de cada jugadora convocada
    - Stats por jugadora (total turnos asignados)
    """
    entrenador = entrenador_actual()
    partido = Partido.query.filter_by(id=partido_id, entrenador_id=entrenador.id).first_or_404()
    ...
    jugs_data = {}
    for j in jugadoras:
        turnos_asignados = plan.turnos_de_jugadora(j.id)
        jugs_data[j.id] = {
            'id': j.id,
            'nombre': j.nombre,
            'apellido': j.apellido,
            ...
            'minutos_planeados': len(turnos_asignados) * plan.duracion_turno_min,
        }

    # Por cada turno, lista de IDs
    turnos_data = {}
    for n in range(1, plan.cantidad_turnos + 1):
        turnos_data[n] = plan.jugadoras_de_turno(n)
...
def planificador_swap(partido_id):
    """
    Intercambia dos jugadoras en un turno específico:
    - sale_id: jugadora que está EN ese turno y sale
    - entra_id: jugadora que NO está en ese turno y entra
    Valida posición (principal o alternativa).
    """
    entrenador = entrenador_actual()
    partido = Partido.query.filter_by(id=partido_id, entrenador_id=entrenador.id).first_or_404()
    plan = PlanificacionPartido.query.filter_by(partido_id=partido.id).first_or_404()
    ...
    entra_id = int(data.get('entra_id', 0))

    if turno_n < 1 or turno_n > plan.cantidad_turnos:
        return jsonify({'ok': False, 'error': 'Turno inválido'}), 400

    sale = Jugadora.query.filter_by(id=sale_id, entrenador_id=entrenador.id).first()
    entra = Jugadora.query.filter_by(id=entra_id, entrenador_id=entrenador.id).first()
    if not sale or not entra:
        return jsonify({'ok': False, 'error': 'Jugadora no encontrada'}), 404

    # Validar que ambas estén convocadas
    convocadas_ids = {c.jugadora_id for c in partido.convocadas}
    if sale.id not in convocadas_ids or entra.id not in convocadas_ids:
        return jsonify({'ok': False, 'error': 'Una de las jugadoras no está convocada'}), 400

    # Validar swap con la lógica del módulo
    plan_turno_actual = plan.jugadoras_de_turno(turno_n)
    from planificador_logic import validar_swap
    valido, mensaje = validar_swap(plan_turno_actual, '4-3-3', sale, entra)
    if not valido:
        return jsonify({'ok': False, 'error': mensaje}), 400

    # Aplicar swap: borrar asignación de sale y crear de entra
    asign_sale = AsignacionTurno.query.filter_by(
        plan_id=plan.id, jugadora_id=sale.id, turno_numero=turno_n
    ).first()
    if asign_sale:
        db.session.delete(asign_sale)

    db.session.add(AsignacionTurno(
        plan_id=plan.id, jugadora_id=entra.id, turno_numero=turno_n
    ))
    db.session.commit()

    return jsonify({'ok': True, 'mensaje': f'{sale.nombre} ↔ {entra.nombre}'})
