...
def generar_plan_automatico(
    ...
    Caso especial: si no hay arqueras convocadas, el slot de arquera se redistribuye
    a la posición con más jugadoras disponibles, manteniendo siempre 11 en cancha.
    ...
    # ============================================================
    # AJUSTE PARA "SIN ARQUERA"
    # Si no hay arquera convocada, redistribuir el slot a la posición
    # con más jugadoras disponibles (suele ser volante o defensora).
    # ============================================================
    arqueras_disponibles = [p for p in convocadas if p.posicion == Posicion.ARQUERA]
    if not arqueras_disponibles and config_formacion.get(Posicion.ARQUERA, 0) > 0:
        # No hay arquera → contar jugadoras de cada otra posición
        contadores = {}
        for pos in [Posicion.DEFENSORA, Posicion.VOLANTE, Posicion.DELANTERA]:
            disponibles = [p for p in convocadas if p.posicion == pos]
            n_en_formacion = config_formacion.get(pos, 0)
            # Cuántas hay disponibles para rotar más allá del titular
            contadores[pos] = (len(disponibles), n_en_formacion)

        # Elegir la posición que pueda absorber un slot extra:
        # priorizar la que tenga más sobrantes (jugadoras - slots actuales)
        mejor_pos = None
        mejor_sobrantes = -1
        for pos, (disp, n) in contadores.items():
            sobrantes = disp - n
            if sobrantes > mejor_sobrantes:
                mejor_sobrantes = sobrantes
                mejor_pos = pos

        if mejor_pos is None:
            # Edge case extremo: no hay ninguna jugadora de campo
            mejor_pos = Posicion.VOLANTE  # fallback razonable

        # Redistribuir: 0 arqueras, +1 en la posición elegida
        config_formacion[Posicion.ARQUERA] = 0
        config_formacion[mejor_pos] = config_formacion.get(mejor_pos, 0) + 1
