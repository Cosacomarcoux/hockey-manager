...
def repartir_minutos_por_posicion(
    ...
    proporcionalmente a su prioridad P.
    ...
    - El cálculo de prioridad ya se hizo antes (asignar_prioridades).

    Devuelve un dict: {posicion: [Player ordenados por prioridad desc]}
    Mutea cada Player asignando .minutos_teoricos.

    Ejemplo:
    >>> asignar_prioridades(players, K=0.7)
    >>> grupos = repartir_minutos_por_posicion(players, "4-3-3")
    >>> # ahora cada player.minutos_teoricos tiene su valor
    """
    ...
    # Para cada posición, repartir minutos proporcional a P
    for posicion, jugadoras_grupo in grupos.items():
        ...
        suma_prioridades = sum(p.prioridad for p in jugadoras_grupo)
        if suma_prioridades == 0:
            # Edge case: prioridades en cero → reparto equitativo
            minutos_por_jug = minutos_disponibles / len(jugadoras_grupo)
            for p in jugadoras_grupo:
                p.minutos_teoricos = minutos_por_jug
        else:
            for p in jugadoras_grupo:
                p.minutos_teoricos = (p.prioridad / suma_prioridades) * minutos_disponibles

        # Ordenar dentro del grupo por prioridad desc (para uso del controller)
